...
import CustomFileUploader from './ProfileUpload';
    ...
    <motion.div className='w-full' variants={containerVariants} initial="hidden" animate="visible">
      <motion.div className='w-full grid grid-cols-1 md:grid-cols-7 cardcontainer'>
        ...
        <motion.div className='col-span-4' variants={childVariants}>
          <motion.div className='grid grid-cols-1 md:grid-cols-2 gap-4 w-full '>
            <Card label='Genel Katılım' content={<p><span className='text-sm'>% </span>{participation ? <AnimatedNumber value={parseInt(participation)} /> : 0}</p>} />
            <Card label='Başarı Oranı' content={<p><span className='text-sm'>% </span><AnimatedNumber value={100} /></p>} />
            <Card label='Kayıtlı Olduğu Ders' content={<AnimatedNumber value={enrolledEduAmount} />} />
            <Card label='Tamamladığı Ders' content={<AnimatedNumber value={completedEduAmount} />} />
          </motion.div>
        </motion.div>
      </motion.div>
    </motion.div>
